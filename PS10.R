...
Q3 <- ggplot(ipip, aes(x=gender,y=logMedInc, fill=exer, color=exer)) +
  stat_summary(fun.y=mean,  geom="bar", fill=NA, position = position_dodge(width = .9), alpha=.4) +
...
Q4 <- ggplot(ipip.l, aes(x=BMI_cat,y=value,color=gender)) +
  ...
  theme_bw() +
  ggtitle("Trait Score by BMI Category") +
  xlab("BMI Category") +
  ylab("Trait Score")
Q4
...
Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=mean, group=trait, color=trait)) +
  geom_line()+
  ...
  xlab("BMI Category") +
  ylab("Trait Score") +
  ggtitle("Trait score by BMI Category by Gender")
Q5
